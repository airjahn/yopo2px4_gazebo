...
# 只考虑水平速度方向，z 设为 0
	norm_xy = math.hypot(vx, vy)  # = sqrt(vx**2 + vy**2)

	if norm_xy > 1e-6:
    		scale = FIXED_SPEED / norm_xy
    		target_msg.velocity.x = vx * scale
    		target_msg.velocity.y = vy * scale
    		target_msg.velocity.z = 0.0
	else:
    # 原始速度几乎为 0：方向不可靠，直接置零或按你自己逻辑处理
    		print("ooooooooooo")
        ...
        # target_msg.acceleration_or_force.z = msg.acceleration.z
